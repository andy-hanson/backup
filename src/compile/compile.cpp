...
namespace {
	...
	void parse_everything(BlockedList<FileAst>& out, List<Diagnostic>::Builder& diagnostics, Arena& diags_arena, Path first_path, Arena& ast_arena, DocumentProvider& document_provider, PathCache& path_cache) {
	...
	Option<Slice<Ref<const Module>>> get_imports(
		const Slice<ImportAst>& imports, Path cur_path, const Compiled& compiled, PathCache& paths, Arena& arena, List<Diagnostic>::Builder& diagnostics
	) {
