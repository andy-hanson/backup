...
namespace {
	...
	Expression convert_let(const LetAst& ast, ExprContext& ctx, Expected& expected) {
		...
		if (find(ctx.parameters, [name](const Parameter& p) { return p.name == name; }))
			throw "todo";
		ExpressionAndType init = convert_and_infer(*ast.init, ctx);
	...
	Expression convert_seq(const SeqAst& ast, ExprContext& ctx, Expected& expected) {
		if (!ctx.void_type) throw "todo";
		Expression first = convert_and_expect(*ast.first, ctx, ctx.void_type.get());
		Expression then = convert(*ast.then, ctx, expected);
	...
	Expression convert(const ExprAst& ast, ExprContext& ctx, Expected& expected) {
		switch (ast.kind()) {
			...
			case ExprAst::Kind::Seq:
				return convert_seq(ast.seq(), ctx, expected);
